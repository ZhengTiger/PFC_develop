...
```{r}
disease_subtype <- OCD_subtype
gene <- Gene_weight$OCD_Gene
...
ggsave("../../Figure/Figure6/Figure_6B_OCD.pdf", plot = Figure_6B,
...
Figure_6C <- 
  ...
  coord_fixed() +
  scale_color_gradientn(colours = c("lightgray", "white", col_Disease[8]), 
                        limits = c(0,4))
Figure_6C
```
...
MDD_Gene <- str_to_title(Gene_weight$MDD_Gene)
MDD_Gene <- MDD_Gene[MDD_Gene %in% rownames(seu)]
MDD_DEG <- FindMarkers(
  seu, 
  ident.1 = colnames(seu)[seu$Time_Subtype %in% MDD_subtype], 
  ident.2 = colnames(seu)[seu$Time_Subtype %in% ANO_subtype],
  features = MDD_Gene, only.pos = T, logfc.threshold = 1)

ANO_Gene <- str_to_title(Gene_weight$ANO_Gene)
ANO_Gene <- ANO_Gene[ANO_Gene %in% rownames(seu)]
ANO_DEG <- FindMarkers(
  seu, 
  ident.1 = colnames(seu)[seu$Time_Subtype %in% ANO_subtype], 
  ident.2 = colnames(seu)[seu$Time_Subtype %in% MDD_subtype],
  features = ANO_Gene, only.pos = T, logfc.threshold = 1)
```
...
#MDD_DEG <- MDD_DEG[MDD_DEG$avg_log2FC>1.5,]
#MDD_gene <- rownames(MDD_DEG)
#MA_gene <- rownames(ANO_DEG)
...
ggsave("../../Figure/Figure6/Figure_6E_Egr1.pdf", plot = Figure_6E, 
